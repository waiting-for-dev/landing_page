    ...
    describe "after save" do
      context "when campaign monitor is configured" do
        it "add the email to campaign monitor" do
          ...
          expect(CreateSend::Subscriber).to receive(:add).with({api_key: LandingPage.campaign_monitor_api_key}, LandingPage.campaign_monitor_list_id, email, name, [{:Key => 'locale', :Value => I18n.locale.to_s}], true).and_return(nil)
